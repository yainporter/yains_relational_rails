  ...
  describe "class methods" do
    describe ".sort_by_creation" do
      it "sorts families by creation date" do
        expect(Family.sort_by_creation).to eq [@saechao, @jones, @pini, @burnett, @lines, @finders, @porter]

        @schnepf = Family.create!(name: "Schnepf", created_at: 1.day.ago)
        @new_family = Family.create!(name: "New Family")

        expect(Family.sort_by_creation).to eq [ @new_family, @saechao, @jones, @pini, @burnett, @lines, @finders, @porter, @schnepf]
      end
    end
  end
end
